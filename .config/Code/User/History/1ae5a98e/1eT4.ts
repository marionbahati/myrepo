...
export class NewgraphComponent {
  ...
  relationChanged(event: any) {
    this.filterAndCreateLinks(event);
  }
  ...
  createAllLinks() {
    ...
    // Retrieve selected company and relation filter values
    const selectedCompany = this.selected_company;
    const selectedRelation = this.relation_filter;
    const searchTerm = this.searchTerm?.toLowerCase() || '';

    // Find the central company and its D3 node
    const centralCompany = this.data.getCompanies().find(company => company.name === 'Codeschaffer');
    if (!centralCompany) {
    ...
    // Find the searched node based on the search term, if available
    const searchedNode = this.nodes.find(node => node.name.toLowerCase() === searchTerm);

    // Loop through companies to create links
    for (let company of this.data.getCompanies()) {
      let sourceNode = this.getNodeByName(company.name);

      // Handle central node links (skip linking to itself)
      if (company.name === 'Codeschaffer') continue;

      // Loop through company relations and create links
      ...
      // Create link between the central node and the company
      if (company.name !== 'Codeschaffer') {
        let targetNode = this.getNodeByName(company.name);
        if (targetNode && !this.hasLink(links, { source: centralNode, target: targetNode })) {
          links.push({ source: centralNode, target: targetNode });
        }
      }
    }

    // If a searched node exists, filter and show related links
    if (searchedNode) {
    ...
    // Update the links array with all generated links
    this.links = links;
  }
  ...
  filterAndCreateLinks(event: any) {
    let links: d3link[] = [];
    let connectedNodes: any[] = [];
  ...
  private drawGraph(nodes: any, links: any): void {
    const simulation = d3.forceSimulation(nodes)
      ...
      .force('collision', d3.forceCollide().radius(-700))
