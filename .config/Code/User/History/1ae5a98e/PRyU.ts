...
export class NewgraphComponent {
  ...
  drawSearchedLinks(event: any) {
    ...
    // if(ziel_node){
    //   this.beziehungenListe.push('${this.kundeAuswahl.relation_type} :${this.ziel_node.name}');
    // }
  ...
  addFilterAndCreateLinks(event: any) {
    ...
    for (let company of this.data.getCompanies()) {
      ...
      for (let relation of (company?.Relations || [])) {
        let targetNode: any = this.getNodeById(relation.ObjectID);
        if (targetNode) {
          // Add a string combining the relation type and the target company name
    ...
    if (this.relationsList.length === 0) {
      this.relationsList.push(`${this['searchedNode']} ist ein nur kunde von Codeschaffer.Weitere bekannte Beziehungen gibt es nicht`);
    }
