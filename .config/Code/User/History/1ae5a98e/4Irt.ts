...
export class NewgraphComponent {
  ...
  addFilterAndCreateLinks(event: any) {
    ...
    // Create links based on selected filters
    for (let company of this.data.getCompanies()) {
      for (let relation of company?.Relations || []) {
        let targetNode = this.getNodeById(relation.ObjectID);

        // Match relation filter
    ...
    // Filter links if a node was searched
    if (searchedNode) {
      ...
      // Use all links if no node searched
      this.links = links;
    }

    // Update graph for specific company and relation filters
    if (selectedCompany && selectedRelation) {
      const sourceNode = this.nodes.find(node => node.name === selectedCompany);
      const targetNode = this.nodes.find(node => this.relationsList.includes(`${selectedRelation}  : ${node.name}`));

      if (sourceNode && targetNode) {
        this['sourceNode'] = sourceNode;
        this['targetNode'] = targetNode;
        this['updateD3Graph'](sourceNode, targetNode);
      }
    }
  }
