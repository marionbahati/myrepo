...
export class NewgraphComponent {
  ...
  addFilterAndCreateLinks(event: any) {
    ...
    // Create links from company relations
    for (let company of this.data.getCompanies()) {
      for (let relation of company?.Relations || []) {
        let targetNode = this.getNodeById(relation.ObjectID);

        // Check relation type
        if (!selectedRelation || relation.relation_type === selectedRelation) {
          let link = { source: company, target: targetNode };
          if (company && targetNode) links.push(link); // Only add if valid nodes
        }
      }
    }

    // Filter for searched node, or use all links
    if (searchedNode) {
      links = links.filter(link => link.source === searchedNode || link.target === searchedNode);
      connectedNodes = [searchedNode, ...links.map(link => link.source), ...links.map(link => link.target)];
    } else {
      connectedNodes = this.nodes;
    }

    // Update the display
    d3.select('figure#netgraph').selectAll('*').remove();
    this.createSvg();
    this.drawGraph(connectedNodes, links);

    // Style and set up simulation
    const simulation = d3.forceSimulation(connectedNodes)
      .force('center', d3.forceCenter(this.width / 2, this.height / 2))
      .force('charge', d3.forceManyBody().strength(-50)) // Basic repulsion
      .force('link', d3.forceLink(links).id((d: any) => d.index).distance(80))
      .force('collision', d3.forceCollide().radius(20));

    // Update nodes and links on simulation tick
    simulation.on('tick', () => {
      this.svg.selectAll('circle')
        .attr('r', 8) // Visible node size
        .attr('fill', 'steelblue')
        .attr('cx', (d: any) => d.x)
        .attr('cy', (d: any) => d.y);
      ...
      this.svg.selectAll('line')
        .attr('stroke', '#999')
        .attr('stroke-width', 2) // Visible link thickness
