...
export class IndividuellComponent  {
  ...
  //second
  items: string[] = ['marke', 'diagnostik', 'wissenschaft'];
  clickedItems: string[] = [];  
...
// fiststuff
  ...
  addNewObject2() {
   ...
   startshow(itm:any):void{
    const foundObject:any = this.clickedItems.find(obj => itm.name === this.object.name);
    if (foundObject === "marke") {
      this.router.navigate(['/marke']);
    } else {
